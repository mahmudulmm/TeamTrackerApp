...
class TemplateCustomizer {
    constructor({
        ...
        this.settings.defaultHeaderType =
            defaultHeaderType || DEFAULT_LAYOUT_TYPE;
        ...
        this.settings.availableStyles =
            availableStyles || TemplateCustomizer.STYLES;
    ...
    setTheme(themeName, updateStorage = true, cb = null) {
        ...
        this._loadStylesheets(
            {
                [themeUrl]: document.querySelector(
    ...
    setLang(lang, force = false) {
        ...
        for (let i = 0, l = themes.length; i < l; i++) {
            const themeName = themes[i].querySelector(
                'input[type="radio"]'
            ).value;
            themes[i].querySelector(
    ...
    _setup(_container = document) {
      ...
      <label class="form-check-label small" for="${inputName}${nameVal}">${title}</label>
        ...
        // Style
        const styleW = this.container.querySelector(
            ".template-customizer-style"
        );
        const styleOpt = styleW.querySelector(
        ...
        // Theme
        const themesW = this.container.querySelector(
        ...
        // Layout wrapper
        const layoutW = this.container.querySelector(
        ...
        if (
            ...
            if (!this._hasControls("rtl") || !rtlSupport) {
                ...
                directionOpt
                    .querySelector(
                        `input[value="${this.settings.rtl ? "rtl" : "ltr"}"]`
                    )
                    .setAttribute("checked", "checked");
            ...
            if (!this._hasControls("contentLayout")) {
                ...
                contentOpt
                    .querySelector(
                        `input[value="${this.settings.contentLayout}"]`
                    )
                    .setAttribute("checked", "checked");
            ...
            if (!this._hasControls("layoutCollapsed")) {
                ...
                setTimeout(() => {
                    if (document.querySelector(".layout-menu-horizontal")) {
                        layoutCollapsedW.parentNode.removeChild(
                            layoutCollapsedW
                        );
                    }
                }, 100);
            ...
            if (!this._hasControls("layoutNavbarOptions")) {
                ...
                navbarTypeOpt
                    .querySelector(
                        `input[value="${this.settings.layoutNavbarOptions}"]`
                    )
                    .setAttribute("checked", "checked");
                const navbarCb = (e) => {
                    this._loading = true;
